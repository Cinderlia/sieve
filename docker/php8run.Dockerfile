...
COPY config/php.ini /etc/php/8.4/apache2/php.ini
COPY config/php8.conf config/php8.load /etc/apache2/mods-available/

RUN ln -s /etc/apache2/mods-available/php8.load /etc/apache2/mods-enabled/ && ln -s /etc/apache2/mods-available/php8.conf /etc/apache2/mods-enabled/

#COPY config/php.ini /etc/php/5.5/cli/php.ini

#RUN ln -s /etc/apache2/mods-available/php8.conf /etc/apache2/mods-enabled/php5.conf
...
COPY --chown=sv:sv  config/phpinfo_test.php config/db_test.php config/cmd_test.php config/run_segfault_test.sh /app/
...
RUN cat /root/py_aff.alias >> /home/sv/.bashrc
...
COPY --from=sieve/php7build /usr/local/lib/php/extensions/no-debug-non-zts-20240924/ast.so /usr/local/lib/php/extensions/no-debug-non-zts-20240924/
